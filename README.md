...
[![codecov](https://codecov.io/gh/eitatech/scaffold-ca/branch/main/graph/badge.svg)](https://codecov.io/gh/eitatech/scaffold-ca)
[![GitHub license](https://shields.io/badge/license-Apache%202-blue)](https://github.com/eitatech/scaffold-ca/blob/main/LICENSE)
[![Scorecards supply-chain security](https://github.com/eitatech/scaffold-ca/actions/workflows/scorecards-analysis.yml/badge.svg)](https://github.com/eitatech/scaffold-ca/actions/workflows/scorecards-analysis.yml)
...
To use the [plugin](https://plugins.gradle.org/plugin/tech.eita.cleanArchitecture) you need Gradle version 6.9 or later, to start add the following section into your **build.gradle** file.

```groovy
plugins {
    id "tech.eita.cleanArchitecture" version "2.2.4"
}
```
...
To use the [plugin](https://plugins.gradle.org/plugin/tech.eita.cleanArchitecture) you need Gradle version 6.9 or later, to start add the following section into your **build.gradle.kts** file.
...
The **`cleanArchitecture | ca`** task will generate a clean architecture structure in your project, this task has four optional parameters; `package` , `type`, `name` and `coverage`.
   ...
   - **`package`** `= <package.we.need>`: You can specify the main or default package of your project. `Default Value = tech.eita`
   ...
   ```shell
   gradle cleanArchitecture --package=tech.eita --type=imperative --name=NameProject --coverage=jacoco --lombok=true
   gradle ca --package=tech.eita --type=imperative --name=NameProject --coverage=jacoco --lombok=true
   ```
...
```shell
  ...
  The **`updateCleanArchitecture | u`** task will update plugin and dependencies in all subprojects, this task has one optional parameter `dependencies` 
  if you only want to update some dependencies the dependency need to contain the group, and the artifact for example for the dependency **cleanArchitecture** you will need to append **tech.eita:cleanArchitecture**.
...
Review the issues, we hear new ideas. Read more [Contributing](https://github.com/eita/scaffold-ms/wiki/Contributing)

<br><br>

# What's Next?

Read more  [About Clean Architecture](https://medium.com/eita-tech/clean-architecture-scaffold-and-mission)
